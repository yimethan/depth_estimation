...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        l_prob, r_prob = self.depth_model(l_inp, r_inp)
...
class DepthBlock(nn.Module):
    ...
    def forward(self, imgl, imgr, carl, carr):
        ...
        left_cost, right_cost = self.cost_volume(imgl0, imgr0) # [1, 64, 128, 64, 64]
    ...
    def cost_volume(self, imgl, imgr, feature_size=32):
        ...
        for d in range(self.maxdisp):
            ...
            if d == 0:
                leftplusright = torch.cat([imgl, imgr], axis=1)
                rightplusleft = torch.cat([imgr, imgl], axis=1)  # feature
                ...
                leftplusright = torch.cat([imgl, rightmove], axis=1)
                rightplusleft = torch.cat([imgr, leftmove], axis=1) # [1, 128, 64, 64]
